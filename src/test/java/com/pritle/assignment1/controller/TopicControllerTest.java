...
public class TopicControllerTest {
	...
	public void topicVotingTest() {
		...
		assertThat("Topic # 1 has 60 votes now (two users, each voted 30 times)", getTopic(1).getVotes(), equalTo(60));

		assertThat("user-1 can't vote anymore", voteForTopic(user1Token, topicIdToVote),
				equalTo(HttpStatus.METHOD_NOT_ALLOWED));

		assertThat("user-2 can't vote anymore", voteForTopic(user2Token, topicIdToVote),
				equalTo(HttpStatus.METHOD_NOT_ALLOWED));
	}
